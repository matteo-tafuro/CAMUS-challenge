...
class VICReg(nn.Module):
    ...
    def forward(self, batch):
        ...
        batch, _batch = batch[:, 0, :, :].unsqueeze(1), batch[:, 1, :, :].unsqueeze(1)

        x = self.projector(self.encoder.forward_ssl(batch))
        y = self.projector(self._encoder.forward_ssl(_batch))
...
def train_vicreg(
    ...
    for epoch in range(epochs):
        ...
        for _batch in range(len(loader)):
            optimizer.zero_grad()
            ...
            if _batch % log_every_n_batches == 0:
                print('Batch {} loss: {}'.format(_batch, loss.item()))
